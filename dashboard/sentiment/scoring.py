"""
Score computation — GDELT tone + event weight + recency decay.
"""
import math
...
def score_article(article):
    ...
    GDELT with tone: tone * 0.6 + event_weight * 0.4
    RSS (no tone): event_weight only
    """
    ...
    # Tone normalization: GDELT -100/+100 to -1/+1
    if tone is not None:
        tone_norm = max(-1.0, min(1.0, tone / 100.0))
        combined = 0.6 * tone_norm + 0.4 * event_weight
    else:
        # RSS fallback: event classification only
...
def compute_city_score(scored_articles):
    ...
    Recency decay: e^(-0.1 * hours_old) — recent articles weigh more.
    """
    ...
    MAX_AGE_HOURS = 14 * 24  # 14 days — older articles excluded from scoring
    ...
    for a in scored_articles:
        ...
        if pub_dt:
            ...
            hours_old = 168  # unknown date assumed 1 week old
    ...
    # Article count = only those passing the filter
    included_count = pos + neg + neu

    # Alert level (based on included articles only)
