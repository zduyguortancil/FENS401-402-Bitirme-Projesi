...
# K: %40 = 120 koltuk (en buyuk segment D'nin WTP araligina uyumlu)
# M: %75 = 225 koltuk (genis bant, LF %85'e kadar acik)
# Y: %100 = sinir yok
...
# Bolge fiyat faktorleri — verideki avg_fare/base_price oranlarindan turetilmis
# Afrika 0.89, Avrupa 0.93, Asya 0.98, Ortadogu 1.03, Amerika 1.07
REGION_FACTORS = {
    "Europe": 0.93, "Middle East": 1.03, "Africa": 0.89,
    "Asia": 0.98, "Americas": 1.07,
}
...
class PricingEngine:
    ...
    # ── ANA FONKSIYON ─────────────────────────────────────────
    def compute_price(self, inventory, dtd, segment_id=None, session_info=None, predicted_remaining=None, competitor_prices=None):
        """
        ...
        # Rekabet baskisi carpani
        competitive_mult = self._competitive_multiplier(competitor_prices, base_price)

        # Bilesik carpan
        combined = supply_mult * demand_mult * sentiment_mult * customer_mult * competitive_mult
        ...
        return {
            ...
            "multipliers": {
                ...
                "competitive": round(competitive_mult, 4),
    ...
    def _supply_multiplier(self, lf, remaining, dtd, inventory, predicted_remaining=None):
        ...
        # Supply carpani sadece DINAMIK fiyatlama sinyali — baseline fiyati sisirmez.
        # Verideki fiyatlar zaten LF etkisini iceriyor, bu carpan FARK uretir.
        ...
        # ── FALLBACK: LF heuristic ──
        # Kalibre edilmis degerler: verideki max/avg fare oraniyla uyumlu
        # max_fare/avg_fare ~ 2.1x (veriden). LF95+ en yuksek fiyat.
        # Supply carpani 1.0 (bos) → 2.0 (dolu) arasinda
        if lf >= 0.95:
            lf_mult = 2.00
        elif lf >= 0.85:
            lf_mult = 1.50
    ...
    def _demand_multiplier(self, dep_date, route, cabin):
        ...
        # Dampening: verideki fiyatlar zaten sezonsellik iceriyor,
        # bu carpan sadece FARK sinyali. 0.4 = hafif etki.
        return 1.0 + (raw - 1.0) * 0.4
    ...
    # ── REKABET BASKISI CARPANI ─────────────────────────────────
    def _competitive_multiplier(self, competitor_prices, our_base_price):
        """Rakip fiyatlarina gore fiyat ayarlamasi.
        Ucuz rakip varsa hafif indirim, pahali rakip varsa hafif zam."""
        if not competitor_prices:
            return 1.0
        cheapest = min(competitor_prices.values())
        ratio = cheapest / our_base_price if our_base_price > 0 else 1.0
        if ratio < 0.80:
            return 0.95
        elif ratio < 0.95:
            return 0.98
    ...
    def _get_open_fares(self, dtd, lf, demand_pressure=1.0, inventory=None):
        ...
        # Kota + ever_closed BURADA YAPILMAZ.
        # Fare class availability sadece DTD + LF kurallarindan olusur.
        # Kota ve kalici kapatma _update_prices'da (simulation_engine) yonetilir.
        # Bu, _process_bot icindeki compute_price cagrilarinin
        # fare class state'ini degistirmesini engeller.
