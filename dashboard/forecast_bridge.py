...
class ForecastBridge:
    ...
    def __init__(self, tft_predictions_df, twostage_clf, twostage_reg, twostage_features,
                 pickup_model, pickup_features, route_meta=None, n_flights_map=None):
        """
        ...
        n_flights_map: dict {entity_id: avg_n_flights} — TFT tahminini ucus basina bolmek icin
        """
        ...
        self._n_flights_map = n_flights_map or {}

        # TFT cache: (entity_id, date_str) → predicted (UCUS BASINA)
        self._tft_cache = {}
        if tft_predictions_df is not None:
            for _, row in tft_predictions_df.iterrows():
                ...
                # TFT rota-gun toplami tahmin eder, simulator tek ucus gorur
                # n_flights'a bolerek ucus basina talebe ceviriyoruz
                n_flights = self._n_flights_map.get(row["entity_id"], 1.0)
                self._tft_cache[(row["entity_id"], ds)] = raw_pred / max(n_flights, 1.0)
    ...
    def get_tft_band(self, route, cabin, dep_date, dtd):
        ...
        entity_id = f"{route.replace('-', '_')}_{cabin}"
        meta = self.route_meta.get(entity_id, {})
        cap_approx = meta.get("capacity", 300 if cabin == "economy" else 49)
