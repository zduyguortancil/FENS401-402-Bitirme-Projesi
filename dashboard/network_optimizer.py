...
class NetworkOptimizer:
    ...
    def __init__(self, route_distances, segments, connecting_pcts=None):
        ...
        self.class_demand = self._estimate_class_demand()  # default (dtd=None)

    def _estimate_class_demand(self, dtd=None):
        """Segment WTP'den fare class bazli talep orani turet.
        DTD-conditional: kalkisa yaklastikca Y share artar (is yolcusu gec gelir).
        Talluri & van Ryzin (2004) Ch 2.4: time-varying EMSR."""
        class_shares = {"V": 0.0, "K": 0.0, "M": 0.0, "Y": 0.0}

        # DTD'ye gore segment-fare mapping degisir
        if dtd is not None and dtd < 14:
            # Son 2 hafta: is yolcusu (A) ve premium (F) agirligi artar
            # A ve F'nin Y'ye yonelimi 3x artar
            seg_map = {
                "A": {"Y": 0.80, "M": 0.20},   # normalde Y:0.50, M:0.50
                "B": {"M": 0.50, "K": 0.50},
                "C": {"Y": 0.30, "M": 0.50, "K": 0.20},  # kongre de Y'ye kayar
                "D": {"K": 0.40, "V": 0.60},
                "E": {"V": 1.00},
                "F": {"Y": 1.00},
            }
        elif dtd is not None and dtd < 30:
            # 2-4 hafta: orta gecis
            seg_map = {
                "A": {"Y": 0.65, "M": 0.35},
                "B": {"M": 0.45, "K": 0.55},
                "C": {"M": 0.40, "K": 0.60},
            ...
            # Erken donem (30+ gun): orijinal dagilim
            seg_map = {
                "A": {"Y": 0.50, "M": 0.50},
                "B": {"M": 0.40, "K": 0.60},
                "C": {"M": 0.40, "K": 0.60},
    ...
    def compute_protection_levels(self, base_price, capacity, current_sold=0,
        ...
        # DTD-conditional fare class talep dagilimi
        class_demand = self._estimate_class_demand(dtd=dtd)
